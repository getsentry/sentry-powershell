...
- `Write-SentryLog` no longer requires an explicit opt-in: logs are sent without setting `$_.EnableLogs = $true`. The option remains available but no longer gates `Write-SentryLog` ([#137](https://github.com/getsentry/sentry-powershell/pull/137))
